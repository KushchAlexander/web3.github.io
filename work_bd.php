...
$pass = '5656523';
$db = new PDO(
    'mysql:host=localhost;dbname=u67298',
    $user,
    $pass,
...
try {
    ...
    if (!preg_match('/^[А-ЯЁёа-я\s]+$/u', $login)) {
        ...
        $stmt = $db->prepare("INSERT INTO user_login (full_name, phone,email,birth_date,gender,bio,contract_agreed) VALUES (:full_name, :phone,:email,:birth_date,:gender,:bio,:contract_agreed)");
        $login = $_POST['login'];
        $email = $_POST['email'];
        $tel = $_POST['tel'];
        $date = $_POST['date'];
        ...
        foreach ($Languages as $language_name) {
            $stmt = $db->prepare("INSERT INTO languages (user_id, language_name) VALUES (:user_id,:language_name)");
